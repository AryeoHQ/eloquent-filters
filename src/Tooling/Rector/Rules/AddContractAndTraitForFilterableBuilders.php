...
class AddContractAndTraitForFilterableBuilders extends AbstractRector
{
    ...
    public function refactor(Node $node): ?Node
    {
        ...
        $usesFiltersRequests = $this->usesFiltersRequestsTrait($node);
        ...
        if ($hasFilterAttributes) {
            ...
            if (! $usesFiltersRequests) {
                $node = $this->addFiltersRequestsTrait($node);
                $hasChanges = true;
            }
        }

        // Rule 2: If FiltersRequests trait is used, add Filterable contract if missing
        if ($usesFiltersRequests && ! $implementsFilterable) {
            $node = $this->addFilterableContract($node);
            $hasChanges = true;
        }

        // Rule 3: If Filterable contract is implemented, add FiltersRequests trait if missing
        if ($implementsFilterable && ! $usesFiltersRequests) {
            $node = $this->addFiltersRequestsTrait($node);
            $hasChanges = true;
        }
    ...
    public function usesFiltersRequestsTrait(Class_ $node): bool
    {
        if ($node->stmts === []) {
            return false;
        }
        ...
        foreach ($node->stmts as $stmt) {
            if ($stmt instanceof TraitUse) {
                foreach ($stmt->traits as $trait) {
                    if ($trait instanceof FullyQualified && $trait->toString() === FiltersRequests::class) {
                        return true;
                    }

                    if ($trait->toString() === 'FiltersRequests') {
                        return true;
                    }
                }
            }
    ...
    private function addFiltersRequestsTrait(Class_ $node): Class_
    {
        // Check if trait is already used
        if ($this->usesFiltersRequestsTrait($node)) {
            return $node;
        }

        // Add use statement for FiltersRequests trait
        // Only add use import if we have a current file context (not in tests)
        try {
            $this->useNodesToAddCollector->addUseImport(
                new FullyQualifiedObjectType(FiltersRequests::class)
            );
        } catch (Throwable $e) {
            // In test environments, UseNodesToAddCollector might not have a current file
            // This is expected and we can continue without adding the use statement
        ...
        $filtersRequestsTrait = new Name('FiltersRequests');
        $traitUse = new TraitUse([$filtersRequestsTrait]);
