...
final class FilteringRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if ($this->usesFiltersRequestsTrait($node) === false) {
            $errors[] = RuleErrorBuilder::message('Classes with Filter attributes must use the Support\Database\Eloquent\FiltersRequests trait.')
    ...
    private function usesFiltersRequestsTrait(Class_ $node): bool
    {
        if ($node->stmts === []) {
            return false;
        }
            ...
            if ($stmt instanceof Node\Stmt\TraitUse) {
                foreach ($stmt->traits as $trait) {
                    if ($trait instanceof FullyQualified) {
                        if ($trait->toString() === FiltersRequests::class) {
                            return true;
                        }
                    }
                }
